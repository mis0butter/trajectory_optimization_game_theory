...
function equality_constraints(Δτ::T, Δv_vec::AbstractMatrix{T}, x̄₀, x̄f₀, N) where T<:Real
    h_vec = zeros(T, 3)
...
function inequality_constraints(Δτ::T, Δv_vec::AbstractMatrix{T}, x̄₀, x̄f₀, N) where T<:Real
    ψ_vec = zeros(T, N+1)
